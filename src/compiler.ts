...
//   - Sticky regex ($r.lastIndex=pos,$r.test(p)) benchmarks ~53% slower than
//     startsWith+boundary due to regex-engine overhead; avoided here.
  ...
  if (node.static) {
    ...
    for (const key in node.static) {
      ...
      while (
        ...
        !deepNode.wildcard &&
        !deepNode.methods &&
        Object.keys(deepNode.static).length === 1
      ) {
        const nextKey = Object.keys(deepNode.static)[0];
        fullKey += "/" + nextKey;
